...
// defines, project
#define MEMCOPY_ITERATIONS 1267000
...
void testBandwidth(unsigned int start, unsigned int end, unsigned int increment,
  ...
  switch (mode) {
    case QUICK_MODE:
      testBandwidthQuick((unsigned int)DEFAULT_SIZE, kind, printmode, memMode,
...
void testBandwidthQuick(unsigned int size, memcpyKind kind, printMode printmode,
                        memoryMode memMode, int startDevice, int endDevice,
                        bool wc) {
  testBandwidthRange(size, size, (unsigned int)DEFAULT_INCREMENT, kind, printmode,
                     memMode, startDevice, endDevice, wc);
}
...
void testBandwidthShmoo(memcpyKind kind, printMode printmode,
  ...
  unsigned int count = 
      1 + (unsigned int)(SHMOO_LIMIT_20KB / SHMOO_INCREMENT_1KB) +
      (unsigned int)((SHMOO_LIMIT_50KB - SHMOO_LIMIT_20KB) / SHMOO_INCREMENT_2KB) +
      (unsigned int)((SHMOO_LIMIT_100KB - SHMOO_LIMIT_50KB) / SHMOO_INCREMENT_10KB) +
      (unsigned int)((SHMOO_LIMIT_1MB - SHMOO_LIMIT_100KB) / SHMOO_INCREMENT_100KB) +
      (unsigned int)((SHMOO_LIMIT_16MB - SHMOO_LIMIT_1MB) / SHMOO_INCREMENT_1MB) +
      (unsigned int)((SHMOO_LIMIT_32MB - SHMOO_LIMIT_16MB) / SHMOO_INCREMENT_2MB) +
      (unsigned int)((SHMOO_MEMSIZE_MAX - SHMOO_LIMIT_32MB) / SHMOO_INCREMENT_4MB);
...
float testDeviceToHostTransfer(unsigned int memSize, memoryMode memMode,
  ...
  bandwidthInGBs = (memSize * (float)MEMCOPY_ITERATIONS) / (float)1e9;
...
float testHostToDeviceTransfer(unsigned int memSize, memoryMode memMode,
  ...
  unsigned char *h_cacheClear1 = (unsigned char *)malloc((size_t)CACHE_CLEAR_SIZE);
  unsigned char *h_cacheClear2 = (unsigned char *)malloc((size_t)CACHE_CLEAR_SIZE);
